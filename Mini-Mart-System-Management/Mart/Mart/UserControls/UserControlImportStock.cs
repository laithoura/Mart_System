...
namespace Mart.UserControls
{
    ...
    public partial class UserControlImportStock : UserControl
    {
        ...
        {
            Exporter.DataGridViewToExel(dgvImport);
        }
        ...
        private void LoadSoldData(LoadFunction load)
        {
            ...
            {                
                ...
                if (searchFor == 1)
                {
                    ...
                    /*Instock Quantity Column*/
                    dgvImport.Columns[lastCol - 4].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight;
                    dgvImport.Columns[lastCol - 4].HeaderText = "In-Stock Quantity";

                    /*Product Name Column*/           
                    dgvImport.Columns[lastCol - 5].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleLeft;

                    /*Product ID Column*/
                    dgvImport.Columns[lastCol - 6].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;

                    foreach (DataGridViewRow row in dgvImport.Rows)
                    {
                        if((int)row.Cells[lastCol -4 ].Value == 0)
                        {
                            row.Cells[lastCol - 4].Style.ForeColor = Color.Red;
                        }
                    }
                }
                else
